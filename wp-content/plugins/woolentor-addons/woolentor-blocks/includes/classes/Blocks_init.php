...
class Blocks_init {
    ...
    public function prepare_block_data( $block ){
        ...
        $block_dir   = WOOLENTOR_BLOCK_PATH . '/src/blocks/';
        ...
        foreach ( $block_list as $block_key => $block ) {
            if( is_array( $block ) ){
                ...
                if( $block['active'] === true && woolentorBlocks_get_option( $block_key, 'woolentor_gutenberg_tabs', 'on' ) === 'on' ){
                    if( ! isset( $block['server_side_render'] ) && file_exists( WOOLENTOR_BLOCK_PATH . '/src/blocks/' . $block_name . '/index.php' ) ){
                        include( WOOLENTOR_BLOCK_PATH . '/src/blocks/' . $block_name . '/index.php' );
                    }else{
