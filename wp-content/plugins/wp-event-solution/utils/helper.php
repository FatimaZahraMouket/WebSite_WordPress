...
class Helper {
     ...
     */
    public static function post_data_query( $post_type, $count = null, $order = 'DESC', $term_arr = null, $taxonomy_slug = null, $post__in = null, $post_not_in = null, $tag__in = null, $orderby_meta = null, $orderby = 'post_date', $filter_with_status = null ) {
    ...
    public static function woocommerce_ticket_widget( $single_event_id, $class = "" ) {
        ...
        if ( is_array( $ticket_list ) && !empty( $ticket_list ) ) {
            ...
            if( !empty( $deadline ) ){
                ...
                $event_expire_date_time = trim( $deadline_new . " " . $etn_start_time );

                $event_timezone = get_post_meta( $single_event_id, "event_timezone", true );
                if ( str_contains($event_timezone, 'UTC+') || str_contains( $event_timezone, 'UTC-') ) { 
                    $time_cal = self::time_calculation( $event_timezone );
                    
                    $dt     = date("Y-m-d H:i:s", strtotime("$event_expire_date_time"));
                    $now_dt = date("Y-m-d H:i:s", $time_cal);
                } else {
                    date_default_timezone_set( $event_timezone );
                    $dt      = date("Y-m-d H:i:s", strtotime("$event_expire_date_time $event_timezone"));
