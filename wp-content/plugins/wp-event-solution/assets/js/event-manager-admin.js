jQuery(document).ready(function ($) {
    ...
    $('.etn-settings-nav li a').on('click', function(){
        ...
        //set hash link
        let subtab_href = $(this).data("id");
        localStorage.setItem('subtab_href', JSON.stringify({ subtab_href: subtab_href }));
...
timePicker({selector: "#etn_end_time,#etn_shedule_start_time,#etn_shedule_end_time",altInputClass:"etn-form-control etn_end", onCloseSelector:"#etn_end_time", onCloseAttr: "data-end_time"})
    ...
    if ($('.etn-settings-dashboard').length > 0) {
        ...
        if (locationHash && $(`.etn-tab li a[href='${locationHash}']`)[0]) {
            ...
            var sub_tab_get_href = localStorage.getItem('subtab_href');
            if( sub_tab_get_href !== null){

                $(`.etn-settings-nav li a`).removeClass("etn-settings-active");
                $(`.etn-settings-tab-content .etn-settings-tab`).css("display", "none");

                var getSubTabId = JSON.parse(sub_tab_get_href);
                $(`.etn-settings-nav li a[data-id='${getSubTabId['subtab_href']}']`).addClass("etn-settings-active");
                $(`.etn-settings-tab-content .etn-settings-tab[id='${getSubTabId['subtab_href']}']`).css("display", "block");
            }
    ...
    $(document).on('click', ".etn-tab > li > a", function (e) {
        ...
        $(`.attr-tab-pane[data-id='${data_id}'] .etn-settings-tab-content .etn-settings-tab:first-child`).css("display", "block");
