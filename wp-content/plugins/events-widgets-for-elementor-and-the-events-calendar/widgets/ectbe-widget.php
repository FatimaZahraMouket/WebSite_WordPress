			...
			$this->add_group_control(
				...
				array(
					...
					'fields_options' => array(
						...
						'font_weight' => array( 'default' => 'bold' ),
					),
					'separator'      => 'before',
				)
			);
			...
			$this->add_control(
				...
				array(
					...
					'raw'             => '<div class="ectbe-pro-features">
								<a href="' . esc_url( 'https://eventscalendartemplates.com/the-events-calendar-widgets-for-elementor/' ) . '" target="_blank">' . esc_html__( 'View Demo', 'ectbe' ) . '</a>
							</div>',
	...
	protected function render() {
		...
		if ( $layout == 'calendar' ) {
			...
			if ( $layout == 'list' ) {
				...
				$event_output .= '<div id="ect-minimal-list-wrp-' . esc_attr( rand( 1, 10000 ) ) . '" class="ect-minimal-list-wrapper">';
			}
			if ( ! empty( $all_events ) ) {
				foreach ( $all_events as $event ) {
					...
					<span class="ectbe-rate" itemprop="price" content="' . esc_attr( tribe_get_cost( $event_id, false ) ) . '">' . esc_attr( tribe_get_cost( $event_id, true ) ) . '</span>
					<meta itemprop="priceCurrency" content="' . esc_attr( tribe_get_event_meta( $event_id, '_EventCurrencySymbol', true ) ) . '" />
					</div>';
					 endif;
					// Address
				...
				<!-- Venue Display Info -->
					...
					<meta itemprop="name" content="' . esc_attr( tribe_get_venue( $event_id ) ) . '">
                ...
                <span class="ectbe-icon"><i class="ectbe-icon-ticket" aria-hidden="true"></i></span>
                <span class="ectbe-rate" itemprop="price" content="' . esc_attr( tribe_get_cost( $event_id, false ) ) . '">' . tribe_get_cost( $event_id, true ) . '</span>
                <meta itemprop="priceCurrency" content="' . esc_attr( tribe_get_event_meta( $event_id, '_EventCurrencySymbol', true ) ) . '" />
                </div>';
					require ECTBE_PATH . 'widgets/layouts/ectbe-' . $layout . '.php';
				}
